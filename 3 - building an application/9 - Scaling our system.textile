...
Because our system is stateless, we can scale it by caching copies of our resource representation in layers staying between the client and the server, typical proxies that are shared by multiple users, and the client tier itself, local copies stored in a local cache.

In the human web cache is controlled through HTTP cache headers, and resource representation copies are stored both in remote proxies and the local browser.

<blockquote>TODO image here</blockquote>

The amazing power that HTTP gives us when correctly making use of such headers is potentialized due to possibility of inserting remote shared caches (proxies) anywhere, as just prior to entering a company's internal network:

<blockquote>TODO image here</blockquote>

Or even within the tier that provides the services, adding a cache layer for all users:

<blockquote>TODO image here</blockquote>

h2. Restfulie local cache
...
<blockquote>TODO image without the request</blockquote>

The first request retrieves the resources:

<pre>GET http://localhost:3000/hotels HTTP/1.1</pre>
...
This and the following possibilities of local caches are supported by default, without any extra effort from the client developer.
...
<pre>GET http://localhost:3000/hotels/94 HTTP/1.1</pre>

And the server response includes the <b>Last-modified</b> header:
...
If-Unmodified-Since: Sat, 16 Jan 2010 23:22:10 GMT</pre>
...
<blockquote>TODO diagram on Last-modified</blockquote>

h3. ETag

Finally, an 'ETag' header on the response gives a identification that works in the same way that 'Last-modified' does, sending a 'If-None-Match' header in the request and receing a "304 Not Modified" response.
...
<pre>GET http://localhost:3000/hotels/94 HTTP/1.1
If-None-Match: "d21f1963230d1b690fc6e60912e8eae8"</pre>
...
<blockquote>TODO diagram on ETag</blockquote>

h2. Conditional getting

Restfulie allows the use of conditional resource retrieval through the use of <b>Etag</b> and <b>Last-modified</b> headers. Once a resource is retrieved, it keeps within its object both reponse values, so it can use them to retrieve the resource again, when required.

Restfulie will use those headers by default if any <b>self</b> relation is invoked as a method, therefore if the server implements an <b>Etag</b> or <b>Last-modified</b> header, the following code would receive a 200 and 304 response in the first and second GET requests, repectively:
...
In order to make it work properly, the server needs to implement the resource rendering process correctly with the two mentioned headers. Fortunately, Restfulie's <b>render_collection</b> method automatically does that for us, therefore our hotels list rendering process already returns a 304 response:
...
In the client, the self invocation will use both the received <b>Etag</b> and <b>Last-modified</b> headers:
...
<blockquote>TODO image with many clients executing request</blockquote>

h2. Benchmarking our system
...
<pre>
	def find_and_book_cheaper(hotels)
...
<pre>
	def find_and_book_cheaper(hotels, attempts = 1)
		return nil if hotels.length == 0
		cheaper = find_cheaper(hotels)
		hotels.remove cheaper
...
A client that successfully books his first attempted hotel will execute a total of 4 http requests, while a client that only succeeds on his <b>n</b>th attempt will execute (1 + 2*n) http requests.
...
<blockquote>TODO add one more step that local cache will help</blockquote>

h2. Benchmarking after turning off local cache and 304
...
<pre>
	TODO code that opens both modules and turn off such support
</pre>
...
h2. Benchmarking with local cache

Now we are ready to run the same test but with support to local caches:

<pre>
	TODO code that opens one module and turn off 304
</pre>
...
h2. Benchmarking with a reverse proxy
...
h2. Benchmarking with a reverse cache
...
<blockquote>TODO image</blockquote>
...
h2. Benchmarking with a load balancer
...
h2. Benchmarking distributed content
...
<blockquote>TODO implement such feature</blockquote>
...
h2. Benchmarking systems exposing published content
...
<blockquote>TODO image with every client's feed</blockquote>

Those client systems support a publisher-subscriber protocol called <b>pubsubhubbub</b> that notifies the aggregation system every time a new hotel is added.
...
<blockquote>TODO image on resource representation expire</blockquote>

<pre>script/generate scaffold hotel name:string uri:string room_count:id expiry_date:datetime</pre>

<blockquote>TODO support pubsubhubbub</blockquote>
