h1. Booking your trip

It is time to move to implement the process through which an user is capable of booking the entire trip package. We will start by allowing users to search for, book and finally paying a flight, and later evolve our system to allow hotel bookings
in the same process as well.

h2. Searching for a flight
...
First of all, let's create the booking class, which will be mapped to a booking resource within the web.

<pre>
script/generate scaffold trip
script/generate migration add_trip_relations
</pre>

<pre>
	TODO add fields for trip to flights and trip to hotels
</pre>

Rails automatically takes care of etag and last_modified values when a resource change its relationships:

<blockquote>TODO test it!</blockquote>
...
Again, Restfulie is smart enough to understand such relations and correctly implement those methods within the Trip if we tell it to do so. In order to see it working, simply <b>do not</b> implement them, but configure Restfulie accordingly:
...
<pre>
	puts trip.to_atom(:controller => Restfulie::Server::MockedController)
</pre>

<pre>
TODO atom representation of a trip, including the first flight
</pre>

Or in typical xml:

<pre>
	puts trip.to_xml(:controller => Restfulie::Server::MockedController)
</pre>

<pre>
TODO xml representation of a trip, including the first flight
</pre>
...
<pre>
trip = Restfulie.at('http://localhost:3000/trips').create
...
h2. Browsing through hotels

h2. Searching for another flight

h2. Connecting to a final hotel

h2. Linked data and information held in other systems

h2. A full booking example
